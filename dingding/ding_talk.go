...
const baseSendURL = "https://oapi.dingtalk.com/robot/send"
...
func NewRobot(accessToken string, secret string) *DingRobot {
	ts := fmt.Sprint(time.Now().UnixNano() / 1e6)
	return &DingRobot{
		AccessToken: accessToken,
		Secret:      secret,
		SendURL: fmt.Sprintf("%s?access_token=%s&timestamp=%s&sign=%s",
			baseSendURL, accessToken, ts, sign(ts+"\n"+secret, secret)),
	}
}
